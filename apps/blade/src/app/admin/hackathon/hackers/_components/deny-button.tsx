...
export default function DenyButton({ hacker }: { hacker: InsertHacker }) {
    ...
    const updateStatus = api.hacker.updateHackerStatus.useMutation({
        onSuccess() {
            toast.success(`Denied ${hacker.firstName} ${hacker.lastName} successfully!`);
            setIsOpen(false);
        },
        onError(opts) {
    ...
    const sendEmail = api.email.sendEmail.useMutation({
        onSuccess: () => {
            toast.success(`Denial email sent to ${hacker.firstName} ${hacker.lastName}!`);
        },
        onError: (opts) => {
            toast.error(opts.message);
        },
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-h-[80vh] overflow-y-auto">
                ...
                <DialogDescription className="text-md">
                    Please confirm the <b className="text-[#FF5E5E]">DENIAL</b> {" "}
