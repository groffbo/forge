...
export default function AcceptButton({ hacker }: { hacker: InsertHacker }) {
    ...
    const updateStatus = api.hacker.updateHackerStatus.useMutation({
        onSuccess() {
            setIsOpen(false);
            toast.success(`Accepted ${hacker.firstName} ${hacker.lastName} successfully!`);
        },
        onError(opts) {
    ...
    const sendEmail = api.email.sendEmail.useMutation({
        onSuccess: () => {
            toast.success(`Acceptance email sent to ${hacker.firstName} ${hacker.lastName}!`);
        },
        onError: (opts) => {
            toast.error(opts.message);
        },
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <Button 
                    className="bg-lime-600 hover:bg-lime-700 p-2"
                    disabled={
            ...
            <DialogContent className="max-h-[80vh] overflow-y-auto">
                ...
                <DialogDescription className="text-md">
                    Please confirm the <b className="text-lime-500">ACCEPTANCE</b> {" "}
